# Back

어린이 행동 습관 형성 웹앱의 Java Servlet 백엔드입니다.
JSP는 백엔드 동선 확인용 최소 화면이며 실제 프론트엔드는 별도로 연결합니다.

## 환경

- Java 21
- Apache Tomcat 9
- Oracle XE 11g
- MyBatis 3.5
- Maven

## 현재 구현

```text
부모 회원가입 및 로그인
-> 아이 프로필과 초대코드 생성
-> 초대코드 아이 로그인
-> 캐릭터 프리셋 및 기본 펫 선택
-> 아이 홈
-> 사진/영상 인증 메타데이터 제출
-> 부모 알림 생성
-> 부모 승인/거절 및 상자 등급 선택
```

AI 이미지 생성, 실제 카메라 파일 업로드, 랜덤 보상 지급은 다음 단계에서
현재 저장 구조와 연결합니다.

## DB 설정

현재 로컬 테스트 계정은 `hr/hr`입니다.

```properties
# src/main/resources/db.properties
db.driver=oracle.jdbc.OracleDriver
db.url=jdbc:oracle:thin:@localhost:1521:xe
db.username=hr
db.password=hr
```

새 Oracle 계정으로 변경할 때 위 파일의 `db.username`, `db.password`를 수정합니다.
환경변수 `DB_URL`, `DB_USERNAME`, `DB_PASSWORD`를 설정하면 파일 값보다 우선합니다.

새 계정에는 다음 SQL을 순서대로 실행합니다.

```text
database/01_parent_schema.sql
database/02_login_child_schema.sql
database/03_game_schema.sql
database/04_frontend_gap_schema.sql
```

## 실행

```powershell
mvn clean package
```

생성 파일:

```text
target/back.war
```

Tomcat 배포 후 접속:

```text
http://localhost:8090/back/login
```

주요 주소:

```text
/back/join
/back/parent/login
/back/parent/dashboard
/back/parent/reviews
/back/child/login
/back/child/setup
/back/child/home
/back/child/mission
```

Oracle XE가 `8080`을 사용하므로 Tomcat은 `8090` 포트를 사용합니다.
